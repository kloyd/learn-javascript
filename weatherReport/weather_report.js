...
function showeatherByCoords(event) {
    event.preventDefault();
    ...
    fetch(apiUrl)
        .then(response => response.json())
        .then(data => {
            const weatherInfo = document.getElementById('weatherByCoords');
            weatherInfo.innerHTML = `<h2>Weather in ${data.name}</h2>
                          <p>Temperature: ${data.main.temp} &#8451;</p>
                          <p>Weather: ${data.weather[0].description}</p>`;
        })
...
document.getElementById('weatherFormByCoords').addEventListener('submit', showeatherByCoords);
